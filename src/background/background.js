  ...
  if (request.action === "createProject") {
    handleCreateProject(request)
      ...
      .catch(error => {
        ...
        sendResponse({ error: error.message || "Failed to create project" });
      });
    return true;
  }
...
async function handleCreateProject(request) {
  ...
  const projectData = {
    ...
    userId: request.data.userId || authUserId,
    isOwned: request.data.isOwned !== undefined ? request.data.isOwned : true
  };
